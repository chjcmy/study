...
### "가상 머신"이라는 단어의 혼란 — JVM vs VMware vs Docker vs venv

"가상 머신"이라는 이름이 여러 기술에 쓰여서 헷갈린다. 각자 해결하는 문제가 다르다.

| 구분 | JVM | VMware/EC2 (VM) | Docker | Python venv |
| --- | --- | --- | --- | --- |
| **가상화 대상** | CPU 명령어 | 하드웨어 전체 | 프로세스+파일시스템 | 패키지 목록 |
| **OS 공유** | O | X | O (커널 공유) | O |
| **목적** | 크로스 플랫폼 실행 | OS 전체 격리 | 서비스 격리 | 패키지 충돌 방지 |
| **코드가 OS에 종속** | X | X | X | O |
...
JVM 자체는 OS마다 따로 설치하지만, **실행하는 .class 파일은 동일**하다. OS 차이를 흡수하는 번역기다.

#### VMware/EC2 — 하드웨어 전체 가상화
...
```text
OS 커널 공유
└── 컨테이너 (파일시스템 + 프로세스 + 네트워크만 격리)
```

맥에서 Docker Desktop을 쓰면 내부적으로 경량 Linux VM을 몰래 띄운다. 도커 컨테이너는 그 위에서 돌아간다.

#### Python venv — 패키지 격리

파이썬 코드는 OS가 직접 실행한다. **OS에 종속**된다. 윈도우에서 만든 `.py`가 리눅스에서 안 될 수도 있다. venv는 패키지 충돌만 막을 뿐 실행 환경을 추상화하지 않는다.

#### JVM ≠ venv인 이유

둘 다 "환경을 격리한다"는 느낌이지만 목적이 다르다.

- **venv**: 같은 OS 안에서 패키지만 분리. 코드는 여전히 OS 종속.
- **JVM**: OS가 달라도 동일하게 실행 보장. 코드가 OS 독립.

> **log-friends와의 연결:** 맥에서 `./gradlew build`로 만든 JAR가 `eclipse-temurin:21-jre-alpine` (리눅스) 컨테이너에서 그대로 실행되는 이유가 JVM 덕분이다. 별도의 리눅스용 빌드가 필요 없다.
...
graph TD
    ...
    JIT["실행 엔진 (JIT 등)"]
    GC["메모리 관리 (GC)"]
    API["Java SE API<br/>(java.lang, java.util, java.io ...)"]
    TOOLS["개발 도구<br/>(javac, javap, jconsole, jvisualvm, jlink ...)"]
...
```mermaid
graph TD
    ROOT["Java 기술 시스템 에디션"]
    SE["Java SE (Standard)<br/>핵심 API + JVM 스펙"]
    EE["Java EE (Enterprise)<br/>SE + 서블릿, JPA, EJB 등"]
    ME["Java ME (Micro)<br/>임베디드 / 모바일 장치"]
    JAKARTA["Jakarta EE<br/>(Eclipse 재단, 2017~)"]
    OPENJDK["OpenJDK 기반<br/>구현체들"]
...
```mermaid
graph LR
    INTERP["인터프리터로 시작<br/>(바이트코드 실행)"]
    PROFILE["핫스팟 탐지<br/>(프로파일링)"]
    C1["C1 컴파일<br/>(빠른 컴파일)"]
    C2["C2 컴파일<br/>(최적화 컴파일)"]
    NATIVE["네이티브 코드"]

    INTERP -->|"실행 중 수집"| PROFILE
    PROFILE -->|"핫 코드 선별"| C1
    C1 --> C2
    C2 --> NATIVE
```
...
graph TD
    GRAAL["GraalVM"]
    GRAAL_JIT["Graal JIT<br/>(Java로 작성된 JIT 컴파일러)"]
    TRUFFLE["Truffle Framework<br/>(언어 인터프리터 프레임워크)"]
    C2["JVM 위에서 C2 대체 가능"]
    POLYGLOT["다중 언어 지원<br/>(JS, Python, Ruby, R, LLVM 기반)"]
    NATIVE_IMG["Native Image (AOT 컴파일)<br/>JVM 없이 단독 실행 가능한 바이너리 생성"]
...
graph TD
    ...
    CLOJURE["Clojure"]
    JRUBY["JRuby"]
    BYTECODE[".class 바이트코드"]
    JVM["JVM (HotSpot)<br/>클래스 로더 → 바이트코드 검증<br/>→ 인터프리터/JIT → 네이티브 실행"]
...
```mermaid
graph TD
    subgraph TRADITIONAL["전통적 스레드 모델 (1:1)"]
        JT1["Java Thread 1"] --> OT1["OS Thread 1"]
        JT2["Java Thread 2"] --> OT2["OS Thread 2"]
        JT3["Java Thread N..."] --> OT3["OS Thread N<br/>(수천 개 → 컨텍스트 스위칭 비용 폭증)"]
    end

    subgraph VIRTUAL["가상 스레드 모델 (M:N)"]
        VT1["Virtual Thread 1"]
        VT2["Virtual Thread 2"]
        VT3["Virtual Thread 3"]
        VTN["Virtual Thread 100만"]
...
```mermaid
graph TD
    JDK21["log-friends-sdk @ JDK 21"]
    LOOM["가상 스레드 (Project Loom)<br/>BatchTransporter의 HTTP ingest 전송이 I/O 바운드<br/>→ 처리량 대폭 향상 가능"]
    ZGC["Generational ZGC<br/>SDK가 대상 앱에 주는 GC 영향 최소화<br/>→ Stop-the-World 감소"]
    PATTERN["Pattern Matching 확장<br/>이벤트 타입별 분기 처리 코드 간결화"]
...
flowchart TD
    START["JVM 시작"]
    PREMAIN["premain() / agentmain() — Instrumentation API"]
    TRANSFORMER["ClassFileTransformer 등록"]
    BYTEBUDDY["클래스 로딩 시 바이트코드 변환 — ByteBuddy"]
    JIT["변환된 바이트코드 JIT 컴파일 — C1/C2"]

    START --> PREMAIN
    PREMAIN --> TRANSFORMER
    TRANSFORMER --> BYTEBUDDY
    BYTEBUDDY --> JIT
```
...
flowchart TD
    SRC["Kotlin 소스 (.kt)"]
    KOTLINC["kotlinc (Kotlin 컴파일러)"]
    CLASS[".class 바이트코드 — Java와 동일한 포맷"]
    LOADER["JVM 클래스 로더가 로드"]
    JIT["핫스팟이 JIT 컴파일"]
    NATIVE["네이티브 코드로 실행"]
...
실제 예제 파일은 `examples/` 아래 `.kts` 스크립트다. 실행은 1장 디렉터리에서 `kotlinc -script examples/<파일명>.kts`로 한다.

- [JvmInfo.kts](examples/JvmInfo.kts): JDK/JRE/JVM, VM 구현체, JVM 인자와 메모리 정보를 출력한다. 1.1의 자바 기술 시스템 구성과 JVM 제품군을 현재 환경으로 확인한다.
- [BytecodeTarget.kts](examples/BytecodeTarget.kts): `data class`, `sealed class`, null-safe 연산을 컴파일 대상으로 만든다. WORA와 JVM 언어 독립성을 `javap`로 확인하는 예제다.
- [VirtualThreadDemo.kts](examples/VirtualThreadDemo.kts): 플랫폼 스레드 풀과 가상 스레드 실행 시간을 비교한다. JDK 21/Project Loom의 동시성 방향을 체감한다.
- [ClassLoaderHierarchy.kts](examples/ClassLoaderHierarchy.kts): Bootstrap/Application 클래스 로더, ClassPath, Module Path를 출력한다. JVM 제품군과 클래스 로더 계층이 런타임에 어떻게 보이는지 연결한다.
- [JitWarmupDemo.kts](examples/JitWarmupDemo.kts): 반복 연산의 라운드별 시간을 찍어 JIT 워밍업을 관찰한다. HotSpot의 적응형 최적화와 계층형 컴파일을 확인한다.
- [GraalNativeHint.kts](examples/GraalNativeHint.kts): GraalVM 여부와 JVM/Native Image 실행 비교 포인트를 출력한다. AOT vs JIT, GraalVM Native Image의 트레이드오프를 다룬다.
