...
5부 예제는 JMM과 락 최적화를 실행 결과로 관찰하는 데 집중한다. 실행은 해당 장 디렉터리에서 `kotlinc -script examples/<파일명>.kts`로 한다.

- [MemoryModelDemo.kts](part5/ch12/examples/MemoryModelDemo.kts): `volatile`, 원자성 경쟁, happens-before, DCL, 명령어 재배열 방지를 확인한다.
- [ThreadSafetyDemo.kts](part5/ch13/examples/ThreadSafetyDemo.kts): `synchronized`, `ReentrantLock`, `AtomicLong`, `LongAdder`, 가상 스레드, `BatchTransporter` 락 구조를 비교한다.

실제 서비스 프로세스에 붙여 볼 때는 예제 출력에서 확인한 스레드 상태를 기준으로 `jstack`을 사용한다.
