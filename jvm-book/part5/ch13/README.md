...
graph LR
  subgraph OS방식["기존 OS 방식"]
    A1["Thread A — 락 보유"] --> B1["Thread B — 락 요청<br/>→ OS 커널 BLOCKED 전환 (비용 큼)"]
    B1 --> C1["Thread A — 락 해제<br/>→ OS가 Thread B 깨움 (비용 큼)"]
  end
  subgraph SPIN방식["스핀 방식"]
    A2["Thread A — 락 보유"] --> B2["Thread B — 락 요청<br/>→ busy-wait 루프 (CPU 사이클 소모)"]
    B2 --> C2["Thread A — 락 해제<br/>→ Thread B 즉시 진입 (컨텍스트 스위칭 없음)"]
  end
```
