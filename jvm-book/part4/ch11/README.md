...
```mermaid
graph TD
  BC[바이트코드] --> INT["인터프리터 (즉시 실행, 프로파일링 수집)"]
  INT -->|핫 코드 감지| JIT[JIT 컴파일러]
  JIT --> C1["C1 (클라이언트 컴파일러)"]
  JIT --> C2["C2 (서버 컴파일러)"]
  C1 --> CC["네이티브 코드 캐시 (Code Cache)"]
  C2 --> CC
```
...
graph TD
  subgraph Block1["Block 1"]
    ...
    LB["b = load y"]
    ADD["c = a + b"]
    IF["if c > 0 goto Block 2"]
    LA --> ADD
    LB --> ADD
    ADD --> IF
  end
  IF -->|true| Block2["Block 2"]
```

**Sea-of-Nodes IR (Graal) — 데이터/제어 단일 그래프:**

```mermaid
graph LR
  LX["LoadNode x"] -->|데이터| ADD[AddNode]
  LY["LoadNode y"] -->|데이터| ADD
  ADD -->|데이터| IF[IfNode]
  IF -->|제어 true| TRUE["[True]"]
  IF -->|제어 false| FALSE["[False]"]
```
...
```mermaid
graph LR
  CALL[원본 메서드 호출] --> PROXY[ByteBuddy 프록시 메서드]
  PROXY --> INTER[인터셉터 로직]
  INTER --> ORIG[원본 메서드]
```
...
log-friends는 `@Service` 메서드 실행 시간이 10ms 이상일 때만 `METHOD_TRACE` 이벤트를 전송한다.

```mermaid
xychart-beta
  title "@Service 메서드 실행 시간 (JIT 워밍업 과정)"
  x-axis ["인터프리터(1)", "인터프리터(2)", "인터프리터(3)", "C1(1)", "C1(2)", "C1(3)", "C2(1)", "C2(2)", "탈최적화(1)", "탈최적화(2)"]
  y-axis "실행 시간 (ms)" 0 --> 40
  bar [35, 30, 28, 18, 14, 12, 5, 4, 22, 8]
  line [10, 10, 10, 10, 10, 10, 10, 10, 10, 10]
```

애플리케이션 시작 직후 JIT 워밍업 전에 불필요한 METHOD_TRACE 이벤트가 다수 발생할 수 있다.
...
1. **메서드 인라인이 "모든 최적화의 어머니"라고 불리는 이유는?** 인라인 후에만 가능해지는 최적화를 3가지 이상 나열하라.

2. **탈출 분석에서 "스택 할당"이 HotSpot에 구현되지 않은 이유는?** 스칼라 치환이 스택 할당보다 우월한 이유를 설명하라.

3. **log-friends의 `@Service` 메서드 임계값(10ms)을 JIT 워밍업 관점에서 평가하라.**

4. **ByteBuddy가 생성한 인터셉터 코드가 JIT 최적화에 미치는 영향을 분석하라.** 프록시 체인이 인라인을 방해하는가?

5. **계층형 컴파일에서 C1으로 컴파일된 코드가 왜 다시 C2로 재컴파일되는가?**

6. **AOT 컴파일이 JIT 컴파일의 피크 성능에 도달하지 못하는 근본적 이유는?**

7. **Graal 컴파일러의 부분 탈출 분석(Partial Escape Analysis)은 어떤 시나리오에서 C2보다 우수한가?**
