# 최대 우도 추정 (MLE): 데이터가 말해주는 최선의 파라미터 찾기
...
- **확률**: **파라미터(모델)가 주어졌을 때**, 특정 데이터가 관찰될 가능성.
...
우도 함수는 파라미터 `θ`(세타)를 변수로 가지는 함수 `L(θ | 데이터)` 입니다. 각 데이터 샘플이 독립적으로 발생했다고 가정하면, 전체 데이터에 대한 우도는 각 데이터 샘플의 확률을 모두 곱한 것과 같습니다.

> `L(θ | 데이터) = P(데이터1 | θ) * P(데이터2 | θ) * ... * P(데이터n | θ)`

동전 던지기 예시로 돌아가 봅시다. 파라미터는 `p`이고, 데이터는 '앞면 7번, 뒷면 3번' 입니다.

> `L(p | 데이터) = p * p * p * p * p * p * p * (1-p) * (1-p) * (1-p) = p^7 * (1-p)^3`
...
이때 사용하는 트릭이 바로 **로그(log)를 취하는 것**입니다. 로그 함수는 단조 증가 함수이므로, 원래 함수를 최대로 만드는 지점과 로그를 씌운 함수를 최대로 만드는 지점이 동일합니다.

- `log(A * B) = log(A) + log(B)` (곱셈이 덧셈으로 바뀜)
...
어떤 함수의 최댓값/최솟값은 그 함수를 미분해서 기울기가 0이 되는 지점에서 나타납니다. 로그 우도 함수를 파라미터 `p`에 대해 미분하고, 그 결과가 0이 되는 `p`를 찾으면 됩니다.
...
많은 머신러닝 모델의 학습 과정은 본질적으로 MLE입니다. 예를 들어, 로지스틱 회귀(Logistic Regression)는 주어진 데이터에 대해 우도를 최대로 만드는 파라미터(가중치)를 찾는 과정입니다. 딥러닝에서 자주 사용되는 손실 함수인 **크로스 엔트로피(Cross-Entropy)를 최소화하는 것은 로그 우도를 최대화하는 것과 수학적으로 동일**합니다. 즉, 모델을 학습시킨다는 것은 데이터의 우도를 최대화하는 파라미터를 찾는 여정이라고 할 수 있습니다.
