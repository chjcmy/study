...
-   **`VACUUM FULL`**: 테이블을 배타적으로 잠그고(Exclusive Lock), 테이블의 내용을 완전히 새로 써서 물리적인 파일 크기 자체를 줄입니다. 매우 느리고 시스템 부하가 크므로, 서비스 중에는 사용을 피해야 하며 꼭 필요한 경우에만 유지보수 시간을 통해 실행해야 합니다.
